...
int		get_time(t_flist **lst, struct stat buff_stat, char *time_tmp)
{
	int	today;
	...
	(*lst)->date = ft_strsub(time_tmp, 4, 7);
	(*lst)->year = ((today - (*lst)->epoc) > 15768000 ?
		ft_strjoin(" ", ft_strsub(time_tmp, 20, 4)) : ft_strsub(time_tmp, 11, 5));
	return (0);
}

int		get_minmaj(struct stat buff_stat, t_flist **lst)
{
	char	*min;
	char	*maj;
	char	*tmp;
	...
	min = ft_itoa(minor(buff_stat.st_rdev));
...
int		file_info(char *path, t_arg *option, t_flist **lst)
{
	...
	if (option->l == 1)
	{
		...
		if ((*lst)->type == 'b' || (*lst)->type == 'c')
			get_minmaj(buff_stat, lst);
